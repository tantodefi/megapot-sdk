...
export async function getLastJackpotResults(): Promise<LastJackpotEvent | undefined> {
    try {
        ...
        const MAX_LOOKBACK = 43200n; // scan up to ~43k blocks back
        const WINDOW = 5000n; // fetch in windows to avoid provider limits

        let currentTo = lastBlock;
        let scanned: bigint = 0n;

        while (scanned <= MAX_LOOKBACK && currentTo >= 0n) {
            const windowSize = currentTo >= WINDOW ? WINDOW : currentTo + 1n;
            const fromBlock = currentTo >= windowSize - 1n ? currentTo - (windowSize - 1n) : 0n;
            ...
            if (events.length > 0) {
                // pick the absolute latest within this window
                events.sort((a, b) => {
                ...
                return {
                    ...
                    winner: e.args.winner as `0x${string}`,
                    winningTicket: Number(e.args.winningTicket),
                    winAmount: e.args.winAmount as bigint,
                    ticketsPurchasedTotalBps: e.args.ticketsPurchasedTotalBps as bigint,
                };
            }

            // advance to previous window
            const advanced = windowSize;
            scanned += advanced;
